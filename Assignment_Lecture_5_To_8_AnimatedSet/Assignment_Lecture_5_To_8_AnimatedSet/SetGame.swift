...
struct SetGame<CardSymbolShape, CardSymbolColor, CardSymbolPattern, NumberOfShapes> where CardSymbolShape: Hashable, CardSymbolPattern: Hashable, CardSymbolColor: Hashable {
    ...
    mutating func formSet(by cards: [Card]) -> Bool {
        ...
        if shapes.count == 2 || colors.count == 2 || patterns.count == 2 || numberOfShapes.count == 2 {
            return false
        }
        return true
    }
